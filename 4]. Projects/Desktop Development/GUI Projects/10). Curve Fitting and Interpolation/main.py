...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def chunk(self):
        ...
        logging.info(f"The user choose the number of chunks as : {chunks}")
        degree = self.degree_slider.value()
        logging.info(f"The user choose the number of degree as : {str(degree)}")
        ...
        if (chunks==1):
            ...
            change = round(1 * len(x_data))
            x = x_data[:change]  # change from last value towards beginning of array
            y = y_data[:change]  # change from last value towards beginning of array

            overlap=int(self.overlap_box.value())
            #overlap=0
            logging.info(f"The user has chosen overlapping percentage :{overlap}")

            if (overlap>=0 and overlap<=25):
                k=int((overlap/100)*((len(x)-1)/chunks))
                n=int((len(x)-1)/chunks)
                x_chunks = list(mit.windowed(x, n=len(x) // chunks, step=n-k))
                y_chunks = list(mit.windowed(y, n=len(x) // chunks, step=n-k))
                self.MplWidget.canvas.axes.clear()
...
######################################### Error Map ##################################
    def error_map(self):
        ...
        for i in range(1,degree+1):
            ...
            errors.extend(
                mean_absolute_error(y_chunks[j], degrees(x_chunks[j]))
        ...
        self.MplWidget2.canvas.axes.set_title(
            f"Percentage Error = {str(rounded_Error)} %", color='r', fontsize=15
        )
